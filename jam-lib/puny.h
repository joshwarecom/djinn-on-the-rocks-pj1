	...
	if (p_width > 24) {
		if (p_width < 30) {
			...
			if (p_width > 66) {
				...
				_PrintSpacesOrMoveBack(42, SCORE__TX);
				print status_field_1;
				_PrintSpacesOrMoveBack(14, MOVES__TX);
			} else {
				if (p_width > 36) {
					! Width is 37-66, print "Score: 0/0"
					_PrintSpacesOrMoveBack(18, SCORE__TX);
				} else {
...
#Endif;
	...
	!FIXME add Karma to status line more elegantly.
	#Ifdef STATUSLINE_SCORE;
		if (_width > 66) {
			_MoveCursor(1, _width-28);
			print (string)KARMA__TX;
			print status_field_3;
		}
	#EndIf;
...
!if(_obj has concealed) print "Is concealed."; else print "Isnt concealed.";
		...
		!FIXME is there a better way to prevent concealed item from showing in inventory? this affects the bottle cork.
		!if(action ~= ##Inv) {
...
!FIXME more hacks to implement karma score
#Ifdef NO_SCORE;
[ main _i _j _copylength _sentencelength _parsearraylength _again_saved _parser_oops _disallow_complex_again;
#Ifnot;
[ main _i _j _copylength _sentencelength _parsearraylength _score _again_saved _parser_oops _disallow_complex_again _karma;
#Endif;
...
#Endif;
		if(parse->1 == 0) {
			!FIXME there MUST be a better way to prevent corrupting the scope when swapping
			_UpdateScope(player, true);
  	  player.justswapped = 0;
			player.swapme = 0;
			James.swapme = 0;
			Child.swapme = 0;
			_ReadPlayerInput();
...
#Ifndef NO_SCORE;
		...
		if(_karma ~= karma && notify_mode == true) {
			PrintMsg(MSG_PARSER_NEW_KARMA, _karma);
		}
#Endif;
