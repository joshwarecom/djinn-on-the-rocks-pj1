...
#EndIf;
	...
	if(scope_stage == 2) {
		...
		scope_objects = 0;
		_risk_duplicates = 1;
		!FIXME find a better way to avoid duplicating cursed objects in scope
		if (p_actor has is_djinn) {
			_risk_duplicates = 0;
		}
	}
...
!FIXME probably a better way to make it so that the cork can be removed directly when bottle is closed.
