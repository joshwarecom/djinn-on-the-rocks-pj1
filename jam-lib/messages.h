...
#EndIf;
#Ifndef MSG_PARSER_NOSUCHTHING_SWAP;
Constant MSG_PARSER_NOSUCHTHING_SWAP "You can only swap cursed things.";
#EndIf;
...
Default MSG_PARSER_NEW_KARMA 123;
#Endif;
...
#Endif;
	...
	MSG_ENTER_NOT_OPEN, MSG_EXIT_NOT_OPEN, MSG_INSERT_NOT_OPEN, MSG_GO_DOOR_CLOSED:
		if (p_arg_1 == Bottle) {
			print "You can't, since ",(the) p_arg_1, " is sealed shut with a cork.";
		}
		else print "You can't, since ",(the) p_arg_1, " is closed.";
...
#Iftrue MSG_PARSER_NEW_SCORE < 1000;
	MSG_PARSER_NEW_SCORE:
		...
		print ".]^";
#Endif;
#Iftrue MSG_PARSER_NEW_KARMA < 1000;
	MSG_PARSER_NEW_KARMA:
		print "^[Your karma has just gone ";
		if(p_arg_1 < karma) {
			p_arg_2 = karma - p_arg_1;
			print "up";
		} else {
			p_arg_2 = p_arg_1 - karma;
			print "down";
		}
		print " by ", p_arg_2, " point";
		if(p_arg_2 > 1) print "s";
		...
		if (player.free == 0 && player.ascended == 0) {
			if (karma >= 100) {
				Ascension();
			}
			else if (karma <= -100) {
