...
[ ExitSub _p;
	_p = parent(player);
	if(parent(_p) == 0) {
    if (player in BroomCloset) {
      <<Enter BroomClosetDoor>>;
    }
    if (player in Pub) {
      <<Go FAKE_E_OBJ>>;
    }
    if (player in RecordsOffice) {
      <<Go FAKE_N_OBJ>>;
    }
    if (player in NorthAlley) {
      <<Go FAKE_S_OBJ>>;
    }
    PrintMsg(MSG_EXIT_ALREADY); rtrue;
  }
	if(noun == 0) <<Exit _p>>;
...
[ FullScoreSub _i _score_sum _sc _ksc _karmascore_sum;
#IfNot;
...
#EndIf;
  ...
  #IfDef KARMATASKS_PROVIDED;
    print "^The following actions affected your Karma:^^";
  	for(_i=0 : _i<NUMBER_KARMATASKS : _i++)
  		if (karmatask_done->_i == 1) {
        _sc = karmatask_scores->(_i);
        switch (_sc) {
          1: _ksc = 25; _karmascore_sum = _karmascore_sum + 25;
          2: _ksc = -25; _karmascore_sum = _karmascore_sum - 25;
          3: _ksc = -50; _karmascore_sum = _karmascore_sum - 50;
          4: _ksc = 50; _karmascore_sum = _karmascore_sum + 50;
        }
  			PANum(_ksc);
  			PrintKarmaTaskName(_i);
  		}
      @new_line;
  PANum(_karmascore_sum);
  print " total (out of between ", MIN_KARMA, " and ", MAX_KARMA, ")^";
  #IfNot;
  	_i = 0; ! Avoid warning
  #EndIf;
...
[ ScoreSub;
  ...
  print ", and accumulated ", karma, " Karma, out of a possible minimum of ", MIN_KARMA, " and maximum of ", MAX_KARMA;
	PrintRank();
];
#Endif;
...
#Endif;
	_obj = parent(player);
	!FIXME find a better way to avoid printing (inside the bottle) in room header descriptions
	if (player hasnt is_djinn) {
