...
[ _ParseToken p_pattern_pointer p_parse_pointer p_phase _noun _i _token _token_type _token_data _old_wn;
	...
	!FIXME there must be a more elegant way to get cursed objects in scope during swapping only
	if (action == ##Swap) {
...
#Endif;
		...
		#EndIf;
						...
						if(inp1 > -1) {
							...
							if (action == ##Swap) {
								PrintMsg(MSG_PARSER_NOSUCHTHING_SWAP);
		...
		if(_best_score == 0) {
			...
			if(parser_unknown_noun_found ~= 0 &&
				...
				if(parser_unknown_noun_found == 0) {
					...
					else PrintMsg(MSG_PARSER_NOSUCHTHING);
				}
			}
		}
		rtrue;
...
.parse_success;
	...
	if(multiple_objects --> 0 == 0) {
		...
		!FIXME definitely probably a better way to make it so that the cork can be directly retrieved from the sealed bottle
		if (second == Bottle) {
			if (noun == Cork) {
				if (Cork in Bottle) {
